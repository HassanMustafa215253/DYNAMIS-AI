...
import random
# import CollisionDetectorVersions.ckdtreeWorking as ct
...
class myEnvironment(arcade.Window):
    ...
    def setup(self):
        ...
        self.dt=1   # 1 year is kept as starting value for dyanmic dt
        ...
        # KELPER 35
        # addCelestial(self.planets,self.sprites_by_id,Name="Planet1",radius=2,position=(-0.084,0),mass=0.8877,velocity=(0,-10.1978))
        # addCelestial(self.planets,self.sprites_by_id,Name="Planet2",radius=2,position=(0.092,0),mass=0.8094,velocity=(0,+9.3025))
        
        # SUN & EARTH
        # addCelestial(self.planets,self.sprites_by_id,Name="Sun",radius=8,position=(2.95e-6,0),mass=1,velocity=(0,+1.917e-5))
        # addCelestial(self.planets,self.sprites_by_id,Name="Earth",radius=2,position=(-0.983,0),mass=3.003e-6,velocity=(0,-6.386))
        ...
        addCelestial(self.planets,self.sprites_by_id,Name="Sun",radius=8,position=(0.000,0),mass=1,velocity=(0,0.002643))
        addCelestial(self.planets,self.sprites_by_id,Name="Earth",radius=1,position=(1,0),mass=3.003e-6,velocity=(0,-6.283))
        addCelestial(self.planets,self.sprites_by_id,Name="Jupiter",radius=4,position=(5.20,0),mass=9.545e-4,velocity=(0,-2.755))
        # addCelestial(self.planets,self.sprites_by_id,Name="",radius=4,position=(5,-3),mass=1.5,velocity=(-3,-2.5))
        
        #3 Body Stable System to test COLLISION MERGER
        # addCelestial(self.planets,self.sprites_by_id,Name="",radius=5,position=(0,5),mass=2,velocity=(0,0))
        # addCelestial(self.planets,self.sprites_by_id,Name="",radius=5,position=(4.3301,-2.5),mass=2,velocity=(0,0))
        # addCelestial(self.planets,self.sprites_by_id,Name="",radius=5,position=(-4.3301,-2.5),mass=2,velocity=(0,0))
        ...
        # for i in range(-5,5):
        #     for j in range(-5,5):
        #         addCelestial(self.planets,self.sprites_by_id,"",radius=3,position=(i*1.1,j*1.1),mass=random.randrange(1,3),velocity=(random.randrange(0,1),random.randrange(0,1)))
        
        #Linear to check triple collision output
        # addCelestial(self.planets,self.sprites_by_id,Name="",radius=5,position=(0,0),mass=2,velocity=(0,0))
        # addCelestial(self.planets,self.sprites_by_id,Name="",radius=5,position=(-20,0),mass=2,velocity=(0,0))
        # addCelestial(self.planets,self.sprites_by_id,Name="",radius=5,position=(20,0),mass=2,velocity=(0,0))
        ...
        self.E_prev=np.empty(2,dtype=np.float64)
        self.E_prev[0]=0
        self.acc_new=np.zeros_like(positions,dtype=np.float64)
    ...
    def on_update(self,delta_time):
        if not self.paused:
            ...
            # if self.print_Debug:
                    # print("i: ", i, "   position -1: ",(self.trail_list[-1][i][0]*AU_p_Pix-global_center_x*AU_p_Pix)," ",(self.trail_list[-1][i][1]*AU_p_Pix-global_center_y*AU_p_Pix),"  position: ",positions[i])                    
            #         print("self.dt: ",self.dt)
            #         print("accelerations 1 : ",accelerations)
            #         print("Position: ",positions)
            #         print("velocity: ",velocities)
            #         print("masses: ",masses,"\n\n")
            # print(f"Tidal Planet 0: {self.Tidal_perturabation(self.planets[0])}")
            # print(f"Tidal Planet 1: {self.Tidal_perturabation(self.planets[1])}")
            # print(f"Tidal Planet 2: {self.Tidal_perturabation(self.planets[2])}")
            # print(f"Angular Momentum: {self.angular_momentum()}, Sum of angular Momentum: {np.sum(self.angular_momentum(),axis=0)}")
            temp=np.sum(self.Total_energy_per_body())
            if temp>-54 and temp<0:
                self.dt=0.00005
                print(f"dt: {self.dt}   Energies: {temp}")
            
            # print(f"pairwise: {self.Pairwise_Energy()}\n")
            
            
            # print(self.calculate_dynamics())
                # print(f"Total Energy: {self.Total_energy_per_body()}, kinetic energy: {self.kinetic_energy_per_body()}, potential energy: {self.potential_energy_per_body()}")
            ...
            accelerations[masses!=0] = self.cal_Acceleration()
            
            # Calculate the new positions of Objects
            positions[masses!=0]= positions[masses!=0]+ velocities[masses!=0]*self.dt + 0.5*accelerations[masses!=0]*self.dt**2 
            ...
            acc_new = np.zeros_like(positions,dtype=np.float64)
            acc_new[masses!=0] = self.cal_Acceleration()  # <-- this is a(t+dt)  required to cal Velocity

            # Calculates the new velocity
            velocities[masses!=0] = velocities[masses!=0] + 0.5*(accelerations[masses!=0] + acc_new[masses!=0])*self.dt
            ...
            # self.E_prev[1]=self.Total_energy()
            # print(f"Total Energy: {self.E_prev[1]}  Change in Energy: {self.E_prev[1]-self.E_prev[0]}")
            # self.E_prev[0]=self.E_prev[1]
            ...
            if self.frame_count% 2== 0:
                ...
                self.scaled_positions[:, 0] = (positions[:, 0] / AU_p_Pix + global_center_x).astype(float)
                self.scaled_positions[:, 1] = (positions[:, 1] / AU_p_Pix + global_center_y).astype(float)
            ...
            if self.time_elapsed >= self.next_snapshot :
                ...
                store_time.append(self.time_elapsed)
                self.next_snapshot += 0.0003
    ...
    def on_close(self):
        ...
        try:
            T = len(store_positions)            # number of timesteps
            N = store_positions[0].shape[0]     # number of objects

            with open("simulation_data.csv", "a") as f:
                f.write("time,object_id,x,y,vx,vy,mass\n")
                print("T :", T, "   N:", N)
                for t in range(T):
                    pos_t = store_positions[t]      # (N, 2)
                    vel_t = store_velocities[t]     # (N, 2)
                    mass = store_masses[t]
                    for i in range(N):
                        x, y = pos_t[i]
                        vx, vy = vel_t[i]
    ...
    def Merger(self):       #FASTER FOR SMALL DATASET
        ...
        for i, j in pairs:
            ...
            # merge physics
            velocities[w] = (mw * velocities[w] + ml * velocities[l]) / M
            positions[w]  = (mw * positions[w]  + ml * positions[l])  / M
            masses[w] = M

            # update radius (volume additive)
            big = self.sprites_by_id[w]
            small = self.sprites_by_id[l]
            new_r = int((big.radius**3 + small.radius**3) ** (1/3))
            big.update_texture(new_r, big.color)
            ...
            self.sprites_by_id[l] = None
            to_delete.add(l)
        
        print("MERGED:",to_delete)

        # remove all dead sprites in one pass (fast)
        if to_delete:
            self.deleted.extend(to_delete)


    def merger2(self):      #SUPPOSEDLY FASTER FOR LARGER DATASET
        tree = ss.cKDTree(positions[masses!=0]) #type: ignore
        collision_radius = 0.275
        ...
        for i, j in pairs:
            ...
            a=self.planets[i].index
            b=self.planets[j].index
    ...
    """def MergerWithJit(self):

        pairs = ct.detect_collisions(positions)  # set of (i,j)
        ...
        win, lose = ct.merge_kernel(pi, pj, masses, positions, velocities)
        self.deleted.extend(lose)
        for i in np.unique(lose):
            self.planets.remove(self.sprites_by_id[i])
            self.sprites_by_id[i]=None
        self.close()
        
        if self.radius_accumulator==True:
            for w in np.unique(win):
                sprite = self.sprites_by_id[w]
                # compute new radius if needed
                sprite.radius=1.1*sprite.radius
                sprite.update_texture(int(sprite.radius), sprite.color)"""
    ...
    def adjust_position_barycenter(self):
        global positions
        sum_of_masses = np.sum(masses,dtype=np.float64)
        center_of_mass = np.sum(positions * masses[:, None], axis=0,dtype=np.float64) / sum_of_masses
    ...
    def adjust_velocity_barycenter(self):
        global velocities
        sum_of_masses = np.sum(masses,dtype=np.float64)
        center_of_mass = np.sum(velocities * masses[:, None], axis=0,dtype=np.float64) / sum_of_masses
    ...
    def cal_Acceleration(self): 
        ...
        diffSqred = (positions[masses!=0][:, np.newaxis, :] - positions[masses!=0][np.newaxis, :, :])
        # [[(x1-x2,y1-y2)]]
        dist = np.sum((diffSqred)**2, axis=2, dtype=np.float64)
        # (x1-x2)^2 + (y1-y2)^2
        ...
        inv_dist3 = np.array(inv_dist3, dtype=np.float64)
        np.fill_diagonal(inv_dist3, 0.0)
        # Removes the Infinity from the matrix
        acc = -G * (diffSqred* inv_dist3[:,:,None] * masses[masses!=0][None,:,None]).sum(axis=1)
    ...
    def kinetic_energy_per_body(self):
        global masses, velocities
        alive = masses != 0

        T_i = np.zeros_like(masses, dtype=float)
        T_i[alive] = 0.5 * masses[alive] * np.sum(velocities[alive]**2, axis=1)
        return T_i

    def potential_energy_per_body(self):
        global masses, positions, softening
        alive = masses != 0
        ...
        for i in range(N):
            for j in range(i+1, N):
                r_vec = pos[i] - pos[j]
                r_soft = np.sqrt(np.sum(r_vec**2) + epsilon)
                U = -G * m[i] * m[j] / r_soft
                V_i[i] += 0.5 * U
                V_i[j] += 0.5 * U

        V_full = np.zeros_like(masses, dtype=float)
        V_full[idx] = V_i
        return V_full
    
    # Total Energy per body is also the Energy from center of mass per body
    # When summed over all bodies it gives total energy of the system
    def Total_energy_per_body(self):    
        T = self.kinetic_energy_per_body()
        V = self.potential_energy_per_body()
        E = T + V
    import numpy as np
    ...
    # ):
    #     """
    #     Compute largest Lyapunov exponent using shadow-trajectory method
    #     with Benettin renormalization.

    #     Parameters
    #     ----------
    #     positions : (N, D) array
    #     velocities : (N, D) array
    #     step       : function(pos, vel) -> pos, vel
    #     dt         : timestep
    #     epsilon    : initial perturbation magnitude
    #     n_steps    : total integration steps
    #     renorm_interval : steps between renormalization

    #     Returns
    #     -------
    #     lambda_est : largest Lyapunov exponent
    #     log_history : log(separation) history
    #     """

    #     # Clone systems
    #     pos_A = positions.copy()
    #     vel_A = velocities.copy()

    #     pos_B = positions.copy()
    #     vel_B = velocities.copy()

    #     # Initial perturbation (single coordinate)
    #     pos_B[0, 0] += epsilon
    ...
    #         pos_A, vel_A = step(pos_A, vel_A)
    #         pos_B, vel_B = step(pos_B, vel_B)

    #         # Renormalize periodically
    #         if step_idx % renorm_interval == 0 and step_idx > 0:

    #             delta = separation(pos_A, pos_B)

    #             # Accumulate logarithmic growth
    #             log_sum += np.log(delta / delta0)
    #             count += 1
    ...
    #             log_history.append(np.log(delta))

    #     # Lyapunov exponent
    #     lambda_est = log_sum / (count * renorm_interval * dt)

    #     return lambda_est, np.array(log_history)    return E
        
    def calculate_dynamics(self):
        pairwise= self.Pairwise_Energy()
        N = pairwise.shape[0]
        visited = [False]*N
        groups = []
        ...
        for i in range(N):
            ...
            while stack:
                a = stack.pop()
                for b in range(N):
                    if not visited[b] and pairwise[a,b] < 0:
                        visited[b] = True
                        stack.append(b)
                        group.add(b)
            groups.append(sorted(group))
        print("Gravitationally Bound Groups:", groups)
