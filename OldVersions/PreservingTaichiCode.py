...
import CollisionDetectorVersions.JAX as wt
...
AU_p_radius=1/2000   # 1 AU = 2000 radius units
G = 39.478    # AU/M⊙/year
...
# ti_pos : Optional[ti.Field]
# ti_vel : Optional[ti.Field]
# ti_acc : Optional[ti.Field]
# ti_mass : Optional[ti.Field]
# ti_radii : Optional[ti.Field]
# ti_ret : Optional[ti.Field]
# ti_idx : Optional[ti.Field]
# ti_change : Optional[ti.Field]
# ti_bound : Optional[ti.Field]
...
class myEnvironment(arcade.Window):
    ...
    def setup(self):
        ...
        for i in range(-5,5):
            for j in range(-5,5):
                addCelestial(self.planets,self.id_to_sprites,"",radius=3,position=(i,j),mass=random.randrange(1,3),velocity=(random.randrange(0,2),random.randrange(0,2)))
        ...
        """Initializing and Binding taichi buffers and initializing collision manager"""
        global ti_pos,ti_vel,ti_acc,ti_mass,ti_radii,ti_ret,ti_idx,ti_change,ti_bound

        # ti_pos=ti.Vector.field(2,ti.f64,shape=len(positions))
        # ti_vel=ti.Vector.field(2,ti.f64,shape=len(velocities))
        # ti_acc=ti.Vector.field(2,ti.f64,shape=len(accelerations))
        # ti_mass=ti.field(ti.f64,shape=len(masses))
        # ti_radii=ti.field(ti.f64,shape=len(positions))
        # ti_ret = ti.Vector.field(2, dtype=ti.i32, shape=len(positions))
        # ti_idx = ti.field(dtype=ti.i32, shape=())
        # ti_change = ti.field(dtype=ti.i32, shape=())
        # ti_bound = ti.Vector.field(2, dtype=ti.f32, shape=2) 
        # CD.bind_buffers(ti_pos,ti_mass,ti_vel,ti_acc,ti_radii,ti_ret,ti_idx,ti_change,ti_bound)
    ...
    def Merger(self):
    #     global positions, velocities, masses, accelerations,ti_pos,ti_vel,ti_acc,ti_mass,ti_radii,ti_ret,ti_idx,ti_change
    #     # print("Positions",positions)
    #     # print("Velocities",velocities)
    #     # print("Masses",masses)
    #     ti_change[None]=0    # type: ignore
    #     """We are using ti_idx to pass the biggest radius of particles as a shortcut else ret_idx is used to store count of collisions
    #    else we would need to make another variable
    #    """
    #     ti_idx[None]=0    # type: ignore
    #     ti_pos.from_numpy(positions)    # type: ignore
    #     ti_vel.from_numpy(velocities)   # type: ignore
    #     ti_acc.from_numpy(accelerations)    # type: ignore
    #     ti_mass.from_numpy(masses)  # type: ignore
    #     for planet in self.planets:
    #         ti_radii[planet.index]=planet.radius    # type: ignore
    #         ti_idx[None]=max(ti_idx[None],planet.radius)   # type: ignore
    #     ti_bound[0]= [ min(positions[:,0]), max(positions[:,0])] # type: ignore
    #     ti_bound[1] =[ min(positions[:,1]), max(positions[:,1])] # type: ignore

    #     CD.collision_manager()
        
    #     if ti_change[None]==1:  # type: ignore
    #         for i in range(ti_idx[None]):  # type: ignore
    #             # print("big index:",int(ti_ret[i].y), "   small index:",int(ti_ret[i].x))   # type: ignore
    #             small=self.id_to_sprites[int(ti_ret[i].x)]    # type: ignore
    #             big=self.id_to_sprites[int(ti_ret[i].y)] # type: ignore
    #             big.update_texture(int(((big.radius**3)+(small.radius**3))**(1/3)),big.color)
    #             self.planets.remove( self.id_to_sprites[int(ti_ret[i].x)] )   # type: ignore
    #             self.id_to_sprites[int(ti_ret[i].x)]=None    # type: ignore

    #         # self.RemovePlanetsFromList(ti_ret.to_numpy()[:ti_idx[None],0]) # type: ignore
            
    #         positions=ti_pos.to_numpy()    # type: ignore
    #         velocities=ti_vel.to_numpy()   # type: ignore
    #         masses=ti_mass.to_numpy()  # type: ignore
    #         accelerations=ti_acc.to_numpy()    # type: ignore
    #         # print("Positions",positions)
    #         # print("Velocities",velocities)
    #         # print("Masses",masses)
        
            
        # Filter to get only valid collisions
