...
AU_p_radius=1/50   # 1 AU = 50 radius units
G = 39.478    # AU/M⊙/year
...
class myEnvironment(arcade.Window):
    ...
    def Merger(self):
            ...
            radii = jnp.array([sprite.radius for sprite in self.planets])

            # Move data to CPU (or just use default device)
            positions_cpu = jax.device_put(positions, device=jax.devices('cpu')[0])
            radii_cpu = jax.device_put(radii, device=jax.devices('cpu')[0])

            # Call the function (stays on CPU)
            pairs, overlaps, valid = wt.spatial_hash_collisions_optimized(positions_cpu, radii_cpu/60)
