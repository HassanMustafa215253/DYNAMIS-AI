...
class myEnvironment(arcade.Window):
    ...
    def setup(self):
        ...
        self.dt=10   # 1 year is kept as starting value for dyanmic dt
    ...
    def kinetic_energy(self):
        ...
        T = 0.5 * np.sum(masses[masses!=0] * np.sum((velocities[masses!=0])**2, axis=1))
        return T
    def potential_energy(self):
        global masses, positions
        N = len(masses)
        V = 0.0
        for i in range(N):
    ...
    #Energy from center of mass per body
    def com_energy_per_body(self):
        global masses, positions, velocities
        total_mass = np.sum(masses)
        v_com = np.sum(velocities * masses[:, None], axis=0) / total_mass
        velocities_rel = velocities - v_com
        
        # Kinetic energy relative to COM
        T_i = 0.5 * masses * np.sum(velocities_rel**2, axis=1)
        ...
        for i in range(N):
            for j in range(i+1, N):
                r = np.linalg.norm(positions[i] - positions[j])
                V_i[i] += -G * masses[i] * masses[j] / r
                V_i[j] += -G * masses[i] * masses[j] / r
