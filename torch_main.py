...
import random
import CollisionDetectorVersions.ptv2 as wt
...
class myEnvironment(arcade.Window):
    ...
    def setup(self):
        ...
        for i in range(-5,5):
            for j in range(-5,5):
                addCelestial(self.planets,self.sprites_by_id,"",radius=3,position=(i,j),mass=random.randrange(1,3),velocity=(random.randrange(0,1),random.randrange(0,1)))
    ...
    def Merger(self):
        radii = torch.tensor([sprite.radius for sprite in self.planets])
        temp= torch.full_like(torch.tensor(masses),-torch.inf,)
        temp[masses!=0] = radii.double()
        pairs, overlaps = wt.spatial_hash_collisions(torch.from_numpy(positions), temp/25)
        
        
        if pairs.shape[0]!=0:
            pairs = wt.find_collision_clusters(pairs)
            ...
            for cluster in pairs:
                ...
                cluster = cluster[masses[cluster] != 0]
                
                if len(cluster) <= 1:
                    continue
                
                big = cluster[masses[cluster].argmax()]
                
                vel = velocities[cluster]
                pos = positions[cluster]
                mas = masses[cluster]
                
                cluster_mass = mas.sum()
                velocities[big] = (vel * mas[:, None]).sum(axis=0) / cluster_mass
                positions[big]  = (pos * mas[:, None]).sum(axis=0) / cluster_mass
                masses[big]     = cluster_mass

                velocities[cluster[cluster != big]] = 0
                positions[cluster[cluster != big]]  = 0
                masses[cluster[cluster != big]]     = 0
                
                total_radius = 0
                
                # print("Index of big: ",big,"  sprite index of big: ",self.sprites_by_id[big].index)
                for i in cluster:
                    if i == big:
                        continue
                    temp = self.sprites_by_id[i]
                    # print("Index of small: ",i,"  sprite index of i: ",temp.index, "type of small: ", type(temp))
                    self.planets.remove(temp)
                    total_radius += temp.radius**3
                    self.sprites_by_id[i] = None
                
                self.sprites_by_id[big].update_texture(
                    int(((self.sprites_by_id[big].radius**3) + total_radius)**(1/3)),
                    self.sprites_by_id[big].color
                )
                self.sprites_by_id[big].radius=int(((self.sprites_by_id[big].radius**3) + total_radius)**(1/3))
                self.deleted.extend(cluster[cluster != big])
    ...
    def update_time_period(self)->None:
        ...
        dt_min_possible=0.0005
        dt_max_possible=0.01
